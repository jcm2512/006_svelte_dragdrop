<script>
  ...
  function dragStartListener(event) {
    ...
    object.setAttribute("offsetWidth", object.offsetWidth / 2);
    object.setAttribute("offsetHeight", object.offsetHeight / 2);
  }

  function dragMoveListener(event) {
    const target = event.target,
      pathTarget = closestPoint(path, [event.clientX, event.clientY]),
      x = pathTarget[0] - target.getAttribute("offsetWidth"),
      y = pathTarget[1] - target.getAttribute("offsetHeight");
